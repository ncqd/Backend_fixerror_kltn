...
public class FileMauGiangVienExporter {
	private XSSFWorkbook workbook;
	private XSSFSheet sheet;

	public FileMauGiangVienExporter() {
	...
	private void writeHeaderLine() throws Exception {
		...
		createCell(row, 0, "DANH SÁCH GIẢNG VIÊN MẪU ", styleHeader);
...
//		createCell(row, 7, "Loại đào tạo: Tiên tiến", styleContent);
		...
		createCell(row, 1, "Mã GV", styleTable);
		createCell(row, 2, "Họ đệm", styleTable);
		createCell(row, 3, "Tên", styleTable);
		createCell(row, 4, "Giới Tính", styleTable);
		createCell(row, 5, "Ngày sinh", styleTable);
		createCell(row, 6, "Số Điện Thoại", styleTable);
		createCell(row, 7, "Nhóm", styleTable);
		createCell(row, 8, "Mã Lớp", styleTable);
		createCell(row, 9, "Học vị", styleTable);
		createCell(row, 9, "CMND", styleTable);
	...
	private void writeDataLines() {
		...
		createCell(row, columnCount++, "11150501", styleContent);
		createCell(row, columnCount++, "Đào Lê Phương", styleContent);
		createCell(row, columnCount++, "Hoa", styleContent);
		createCell(row, columnCount++, "Nữ", styleContent);
		createCell(row, columnCount++, "21/04/1970", styleContent);
		createCell(row, columnCount++, "'0987784417", styleContent);
		createCell(row, columnCount++, "2000", styleContent);
		createCell(row, columnCount++, "tiến sĩ", styleContent);
		createCell(row, columnCount++, "909123123321", styleContent);
